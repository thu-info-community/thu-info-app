...
import {beginTime, endTime} from "./schedule";
...
export const ScheduleAddModal = ({
	...
	const handleSave = () => {
		...
		const newSchedule: Schedule = {
			name:
				numberToCode(customCnt) +
				(title || params?.name?.substring(6) || ""),
			location: locale,
			activeTime: {base: []},
			delOrHideTime: {base: []},
					...
					<RoundedView style={{marginTop: 16, padding: 16}}>
									...
									popupContent={
										<View style={{flexDirection: "row"}}>
											...
											<ScrollPicker
												...
												selectedIndex={
													popupPeriodEnd - popupPeriodBegin
												}
												renderItem={(data) => (
													<Text
														style={{
						...
						{useCustomDateTime && (
							<>
								...
								<BottomPopupTriggerView
									disabled={timeEditable}
									popupTitle={`${String(popupBeginHour).padStart(
										2,
										"0",
									)}:${String(popupBeginMinute).padStart(
										2,
										"0",
									)} - ${String(popupEndHour).padStart(2, "0")}:${String(
										popupEndMinute,
									).padStart(2, "0")}`}
									popupContent={
										<View style={{flexDirection: "row"}}>
