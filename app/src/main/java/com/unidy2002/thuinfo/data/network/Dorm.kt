...
fun Network.getDormScore(): String? =
    retryTemplate(-1) {
        connect(
...
@Synchronized
fun Network.getEleRechargePayCode(money: Int): String? {
    try {
        val validChars =
            ('0'..'9') + ('a'..'z') + ('A'..'Z') + ' ' + '=' + '+' + '-' + '/' + '_' + '(' + ')' + '<' + '>' + ',' + '.' + '\'' + '`' + '~' //*&@!#$%^"
        // TODO: these valid chars might be far from enough
        val tempPassword = loggedInUser.communityPassword.filter { it in validChars }
